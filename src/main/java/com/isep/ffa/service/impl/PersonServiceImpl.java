...
public class PersonServiceImpl extends BaseServiceImpl<PersonMapper, Person> implements PersonService {
    ...
    public PagedResponse<Person> getPage(int page, int size) {
        // Use 1-based page for consistency or adjust based on your BaseService logic
        // Assuming BaseService logic or simple MyBatis-Plus Page
        ...
        return PagedResponse.of(
                result.getRecords(),
                (int) result.getCurrent() - 1, // Convert back to 0-based for frontend if needed
                (int) result.getSize(),
                result.getTotal(),
                (int) result.getPages()
        );
    ...
    public BaseResponse<Person> createPerson(Person person) {
        ...
        if (person.getRoleId() == null) {
            ...
            if (defaultRole != null) {
                ...
                // For now, allow creation without role or handle appropriately
            }
        }
    ...
    public BaseResponse<Boolean> deletePerson(Long id) {
        ...
        // Direct physical delete or logical delete based on your preference
        // Using removeById (typically logical delete if @TableLogic is present on entity)
